...
function goToScene(n){
  ...
  old.classList.remove("active");old.classList.add("exit");
  setTimeout(()=>old.classList.remove("exit"),800);
  next.classList.add("active");currentScene=n;
  if(n===7)startCountdown();
  if(n===8)startTypewriter();
  if(n===10)startFireworks();
}
function startAudio(){
...
// CHAPTER 09 — SECRET MEMORY GAME
let memoryLevel=0,memorySequence=[],memoryInput=[],memoryWatching=false,memoryGameStarted=false;
const memoryButtons=$$(".memory-symbol");
const memorySymbols=["moon","star","heart","spark"];
function flashMemorySymbol(symbol){
  const b=memoryButtons.find(x=>x.dataset.symbol===symbol);
  if(!b)return;
  b.classList.add("lit");
  setTimeout(()=>b.classList.remove("lit"),360);
}
function playMemorySequence(){
  memoryWatching=true;memoryInput=[];
  $("#memory-code-status").textContent="Watch carefully… 👀";
  memorySequence.forEach((symbol,i)=>setTimeout(()=>flashMemorySymbol(symbol),500+i*650));
  setTimeout(()=>{memoryWatching=false;memoryInput=[];$("#memory-code-status").textContent="Now repeat it! ✨"},500+memorySequence.length*650);
}
function startMemoryRound(){
  memoryLevel++;
  memorySequence=Array.from({length:memoryLevel+1},()=>memorySymbols[Math.floor(Math.random()*memorySymbols.length)]);
  $("#memory-level").textContent=`Level ${memoryLevel} / 3`;
  playMemorySequence();
}
function resetMemoryGame(){
  memoryLevel=0;memorySequence=[];memoryInput=[];memoryWatching=false;memoryGameStarted=false;
  $("#memory-level").textContent="Level 0 / 3";
  $("#memory-code-status").textContent="Ready when you are…";
  $("#start-memory").textContent="Start memory game";
}
$("#start-memory").onclick=()=>{
  if(memoryWatching)return;
  if(memoryLevel>=3){resetMemoryGame();}
  memoryGameStarted=true;$("#start-memory").textContent="Memory game running…";
  startMemoryRound();
};
memoryButtons.forEach(button=>button.onclick=()=>{
  if(!memoryGameStarted||memoryWatching||memoryLevel===0)return;
  const chosen=button.dataset.symbol;
  const index=memoryInput.length;
  memoryInput.push(chosen);
  flashMemorySymbol(chosen);
  if(chosen!==memorySequence[index]){
    memoryLevel=0;memorySequence=[];memoryInput=[];memoryWatching=false;
    $("#memory-level").textContent="Level 0 / 3";
    $("#memory-code-status").textContent="Wrong sequence! Try again. 🔐";
    $("#start-memory").textContent="Try again";
    memoryGameStarted=false;
    return;
  }
  if(memoryInput.length===memorySequence.length){
    if(memoryLevel===3){
      memoryWatching=false;memoryGameStarted=false;
      $("#memory-code-status").textContent="Secret unlocked! 🎉";
      $("#start-memory").textContent="Continue →";
      burstConfetti();
      setTimeout(()=>goToScene(10),900);
    }else{
      $("#memory-code-status").textContent="Correct! Next level… ✨";
      setTimeout(startMemoryRound,650);
    }
  }
});
...
$("#message-next").onclick=()=>goToScene(10);
...
function startFireworks(){
  ...
  const loop=setInterval(()=>{if(currentScene!==10){clearInterval(loop);return}launchFirework(innerWidth/2+(Math.random()-.5)*innerWidth*.55,innerHeight*(.22+Math.random()*.35))},1250);
...
function replayFromBeginning(){
  ...
  orbCount=0;giftCount=0;memoryIndex=0;quizIndex=0;countdownStarted=false;typingStarted=false;resetMemoryGame();
